...
/* ───────────────── FETCH REPORTS ───────────────── */
$sql = "
 SELECT r.report_id, r.title, r.location, r.urgency_level, r.status, r.report_date, r.user_id,
        u.username,
...
<body class="d-flex">
  ...
  <div class="content p-4 w-100">
    <div class="d-flex justify-content-between align-items-center mb-3">
      <h2>🛠️ Admin View – All Maintenance Reports</h2>
                  ...
                  <?php if ($row['evidence'] && file_exists($row['evidence'])): ?>
                    <div class="media-container">
                      <?php if ($row['media_type'] === 'video'): ?>
                        <video class="report-video" controls preload="metadata" style="cursor:pointer" data-bs-toggle="modal"
                          data-bs-target="#mediaModal<?= $row['report_id'] ?>">
                          <source src="<?= htmlspecialchars($row['evidence']) ?>" type="video/mp4">
                        </video>
                        <span class="media-type-badge">VIDEO</span>
                      <?php else: ?>
                        <img src="<?= htmlspecialchars($row['evidence']) ?>" class="report-image" data-bs-toggle="modal"
                          data-bs-target="#mediaModal<?= $row['report_id'] ?>">
                      ...
                      <div class="modal-dialog modal-dialog-centered modal-xl">
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            <a href="<?= htmlspecialchars($row['evidence']) ?>" class="btn btn-primary" download>📥
                              Download</a>
